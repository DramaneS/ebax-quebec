...
type RestOf<T extends IProject | NewProject> = Omit<T, 'startDateWork' | 'endDateWork'> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      startDateWork: project.startDateWork?.toJSON() ?? null,
      endDateWork: project.endDateWork?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      startDateWork: restProject.startDateWork ? dayjs(restProject.startDateWork) : undefined,
      endDateWork: restProject.endDateWork ? dayjs(restProject.endDateWork) : undefined,
    };
  }
